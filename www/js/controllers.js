...
.controller('AnalysisCtrl', function($scope, $ionicLoading, $state, DataService) {
  
  console.log("I am in analysis controller");

   
  
})

.controller('UpcomingCtrl', function($scope, $window, $http, $state, DataService) {
  
  $scope.TankDetails = DataService.getTank();

  $scope.sump = {type:"Sump", Sensorid:"wasu0176", capacity:"2L", level:"full"};
  $scope.overhead = {type:"Overhead", Sensorid:"wasu0659", capacity:"1L", level:"full"};

    $http.get("https://api.thingspeak.com/update?api_key=MRBH8MEQXEZQPQFI&field1=0")
    .then(function(response) {
        console.log(response.data);
        $scope.sump.level = response.data;
        
    });
     $http.get("https://api.thingspeak.com/update?api_key=MRBH8MEQXEZQPQFI&field2=0")
    .then(function(response) {
        console.log(response.data);
        $scope.overhead.level = response.data;
